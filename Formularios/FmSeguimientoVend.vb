Public Class FmSeguimientoVend
    ...
    Public Sub Mostrar_Datos()
        Dim Sql As String =
"Select PRO_ID,Rtrim(PRO_NOMBRE)+' '+Rtrim(PRO_PATERNO)+' '+Rtrim(PRO_MATERNO) as NOMBRE ,ISNULL((CASE PRO_STATUS1 WHEN 1 THEN Rtrim(PRO_CLI1)+ ' ' + Rtrim(PRO_MOD1)+' Paq. '+ PRO_PAQ1 END),'')+' '+" &
        ...
        "isnull(ACT_ESTATUSPM,'') as PM,isnull(ACT_ESTATUSGE,'') as GE,isnull(ACC_ESTATUS,'') as ACC,isnull(ACT_ESTATUSFI,'') as FI,isnull(TAC_ESTATUS,'') as TAC, " &
        "isnull(RTRIM(COM_MARCA1),'') + ' ' +isnull(RTRIM(COM_MARCA2),'') + ' ' + isnull(RTRIM(COM_MARCA3),'') as CMP,isnull(ACT_ESTATUSPE,'') as PE,isnull(ACT_ESTATUSPA,'') as PA,isnull(ACT_ESTATUSCRE,'') as CRE,isnull(ENG_ESTATUS,'') as ENG,isnull(ACT_ESTATUSFAC,'') as FAC,isnull(ACT_ESTATUSENT,'') as ENT " &
        "From CRM_PROSPECTOS as PROS LEFT JOIN CRM_VENDEDORES AS VEND ON PROS.PRO_IDvend = VEND.VEND_ID LEFT JOIN CRM_TAC ON PROS.PRO_ID = CRM_TAC.TAC_IDPROS left join CRM_ACTIVIDADES AS ACT ON PROS.PRO_ID = ACT.ACT_IDPROS left join CRM_COMPETIDOR AS COMP ON PROS.PRO_ID = COMP.COM_IDPROS left join CRM_ACCESORIOS AS ACC ON PROS.PRO_ID = ACC.ACC_PROSID LEFT JOIN CRM_ENGANCHE AS ENG ON PROS.PRO_ID = ENG.ENG_IDPROS WHERE isnull(ACT.ACT_ESTATUSFAC,'') <> 'SI' and pros.PRO_IDVEND = '" & Vendedor & "'"
...
For A As Integer = 0 To DvgProsperto.RowCount() - 1 Step 1
            For B As Integer = 5 To 13 Step 1
    ...
    Public Sub VerificoAut()
        ...
        If Conexion2.dr.HasRows Then
            Conexion2.dr.Read()
            If (Conexion2.dr.GetString(0) = "SI") Then FmActividades.ShowDialog() Else MsgBox("El gerente de ventas no ha autorizado la compra.")
            Conexion2.dr.Close()
    ...
    Public Sub VerificoPe()
        ...
        If Conexion2.dr.HasRows Then
            ...
            If (Conexion2.dr.GetString(0) = "SI,SI") Then
                Conexion2.dr.Close()
                Conexion2.desconectarse()
                If (Actividad = "CMP") Then FmCompetidor.ShowDialog() Else FmActividades.ShowDialog()
            Else
    ...
    Public Sub VerificoFAC()
        Conexion2.Consultar("SELECT isnull(ACT_ESTATUSPE,''),isnull(ENG_ESTATUS,''),isnull(ACT_ESTATUSPA,'') FROM CRM_ACTIVIDADES LEFT JOIN CRM_ENGANCHE AS ENG ON CRM_ACTIVIDADES.ACT_IDPROS = ENG.ENG_IDPROS WHERE ACT_IDPROS ='" & Id_Bus & "'")
        If Conexion2.dr.HasRows Then
            Conexion2.dr.Read()
...
"Select PRO_ID,Rtrim(PRO_NOMBRE)+' '+Rtrim(PRO_PATERNO)+' '+Rtrim(PRO_MATERNO) as NOMBRE ,ISNULL((CASE PRO_STATUS1 WHEN 1 THEN Rtrim(PRO_CLI1)+ ' ' + Rtrim(PRO_MOD1)+' Paq. '+ PRO_PAQ1 END),'')+' '+" &
        ...
        "isnull(ACT_ESTATUSPM,'') as PM,isnull(ACT_ESTATUSGE,'') as GE,isnull(ACC_ESTATUS,'') as ACC,isnull(ACT_ESTATUSFI,'') as FI,isnull(TAC_ESTATUS,'') as TAC, " &
        "isnull(RTRIM(COM_MARCA1),'') + ' ' +isnull(RTRIM(COM_MARCA2),'') + ' ' + isnull(RTRIM(COM_MARCA3),'') as CMP,isnull(ACT_ESTATUSPE,'') as PE,isnull(ACT_ESTATUSPA,'') as PA,isnull(ACT_ESTATUSCRE,'') as CRE,isnull(ENG_ESTATUS,'') as ENG,isnull(ACT_ESTATUSFAC,'') as FAC,isnull(ACT_ESTATUSENT,'') as ENT " &
        "From CRM_PROSPECTOS as PROS LEFT JOIN CRM_VENDEDORES AS VEND ON PROS.PRO_IDvend = VEND.VEND_ID LEFT JOIN CRM_TAC ON PROS.PRO_ID = CRM_TAC.TAC_IDPROS left join CRM_ACTIVIDADES AS ACT ON PROS.PRO_ID = ACT.ACT_IDPROS left join CRM_COMPETIDOR AS COMP ON PROS.PRO_ID = COMP.COM_IDPROS left join CRM_ACCESORIOS AS ACC ON PROS.PRO_ID = ACC.ACC_PROSID LEFT JOIN CRM_ENGANCHE AS ENG ON PROS.PRO_ID = ENG.ENG_IDPROS WHERE isnull(ACT.ACT_ESTATUSFAC,'') <> 'SI' and PRO_NOMBRE like '" + txtnombre.Text + "' or PRO_PATERNO like'" + txtpaterno.Text + "' OR PRO_MATERNO like '" + txtmaterno.Text + "'"
        Conexion.LlenarGrilla(Sql, "CRM_PROSPECTOS")
        DvgProsperto.DataSource = Conexion.Dt
        COLOR_CELDAS()
        ColoreaAct()
...
"Select PRO_ID,Rtrim(PRO_NOMBRE)+' '+Rtrim(PRO_PATERNO)+' '+Rtrim(PRO_MATERNO) as NOMBRE ,ISNULL((CASE PRO_STATUS1 WHEN 1 THEN Rtrim(PRO_CLI1)+ ' ' + Rtrim(PRO_MOD1)+' Paq. '+ PRO_PAQ1 END),'')+' '+" &
        ...
        "isnull(ACT_ESTATUSPM,'') as PM,isnull(ACT_ESTATUSGE,'') as GE,isnull(ACC_ESTATUS,'') as ACC,isnull(ACT_ESTATUSFI,'') as FI,isnull(TAC_ESTATUS,'') as TAC, " &
        "isnull(RTRIM(COM_MARCA1),'') + ' ' +isnull(RTRIM(COM_MARCA2),'') + ' ' + isnull(RTRIM(COM_MARCA3),'') as CMP,isnull(ACT_ESTATUSPE,'') as PE,isnull(ACT_ESTATUSPA,'') as PA,isnull(ACT_ESTATUSCRE,'') as CRE,isnull(ENG_ESTATUS,'') as ENG,isnull(ACT_ESTATUSFAC,'') as FAC,isnull(ACT_ESTATUSENT,'') as ENT " &
        "From CRM_PROSPECTOS as PROS LEFT JOIN CRM_VENDEDORES AS VEND ON PROS.PRO_IDvend = VEND.VEND_ID LEFT JOIN CRM_TAC ON PROS.PRO_ID = CRM_TAC.TAC_IDPROS left join CRM_ACTIVIDADES AS ACT ON PROS.PRO_ID = ACT.ACT_IDPROS left join CRM_COMPETIDOR AS COMP ON PROS.PRO_ID = COMP.COM_IDPROS left join CRM_ACCESORIOS AS ACC ON PROS.PRO_ID = ACC.ACC_PROSID LEFT JOIN CRM_ENGANCHE AS ENG ON PROS.PRO_ID = ENG.ENG_IDPROS WHERE isnull(ACT.ACT_ESTATUSFAC,'') = 'SI' "
        Conexion.LlenarGrilla(Sql, "CRM_PROSPECTOS")
        DvgProsperto.DataSource = Conexion.Dt
        COLOR_CELDAS()
        ColoreaAct()
    ...
    Private Sub btnbuscacita_Click(sender As System.Object, e As System.EventArgs) Handles btnbuscacita.Click
        Dim Sql As String =
"Select PRO_ID,Rtrim(PRO_NOMBRE)+' '+Rtrim(PRO_PATERNO)+' '+Rtrim(PRO_MATERNO) as NOMBRE ,ISNULL((CASE PRO_STATUS1 WHEN 1 THEN Rtrim(PRO_CLI1)+ ' ' + Rtrim(PRO_MOD1)+' Paq. '+ PRO_PAQ1 END),'')+' '+" &
        "ISNULL((CASE PRO_STATUS2 WHEN 1 THEN Rtrim(PRO_CLI2)+ ' ' + Rtrim(PRO_MOD2)+' Paq. '+ PRO_PAQ2 END),'') AS VEHICULO ,ISNULL((SELECT TOP(1)CIT_FECCITA  FROM CRM_CITAS where CIT_IDPROS = PRO_ID ORDER BY CIT_FECCITA DESC),PRO_FECALTA) AS CITA, " &
        "isnull(ACT_ESTATUSPM,'') as PM,isnull(ACT_ESTATUSGE,'') as GE,isnull(ACC_ESTATUS,'') as ACC,isnull(ACT_ESTATUSFI,'') as FI,isnull(TAC_ESTATUS,'') as TAC, " &
        "isnull(RTRIM(COM_MARCA1),'') + ' ' +isnull(RTRIM(COM_MARCA2),'') + ' ' + isnull(RTRIM(COM_MARCA3),'') as CMP,isnull(ACT_ESTATUSPE,'') as PE,isnull(ACT_ESTATUSPA,'') as PA,isnull(ACT_ESTATUSCRE,'') as CRE,isnull(ENG_ESTATUS,'') as ENG,isnull(ACT_ESTATUSFAC,'') as FAC,isnull(ACT_ESTATUSENT,'') as ENT " &
        " From CRM_PROSPECTOS as PROS LEFT JOIN CRM_VENDEDORES AS VEND ON PROS.PRO_IDvend = VEND.VEND_ID LEFT JOIN CRM_TAC ON PROS.PRO_ID = CRM_TAC.TAC_IDPROS left join CRM_ACTIVIDADES AS ACT ON PROS.PRO_ID = ACT.ACT_IDPROS left join CRM_COMPETIDOR AS COMP ON PROS.PRO_ID = COMP.COM_IDPROS LEFT JOIN CRM_CITAS AS CIT ON PROS.PRO_ID = CIT.CIT_IDPROS left join CRM_ACCESORIOS AS ACC ON PROS.PRO_ID = ACC.ACC_PROSID LEFT JOIN CRM_ENGANCHE AS ENG ON PROS.PRO_ID = ENG.ENG_IDPROS WHERE isnull(ACT.ACT_ESTATUSFAC,'') <> 'SI' AND CIT.CIT_FECCITA = '" & dtpcita.Value & "'"
        Conexion.LlenarGrilla(Sql, "CRM_PROSPECTOS")
        DvgProsperto.DataSource = Conexion.Dt
        COLOR_CELDAS()
        ColoreaAct()
    ...
    Private Sub btncanccita_Click(sender As System.Object, e As System.EventArgs) Handles btncanccita.Click
        Mostrar_Datos()
    End Sub
End Class