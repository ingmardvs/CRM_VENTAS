...
"Select PRO_ID,isnull(RTRIM(VEND_NOMBRE),'') + ' ' + isnull(RTRIM(VEND_PATERNO),'') + ' ' + isnull(RTRIM(VEND_MATERNO),'') as VEND, Rtrim(PRO_NOMBRE)+' '+Rtrim(PRO_PATERNO)+' '+Rtrim(PRO_MATERNO) as NOMBRE ,ISNULL((CASE PRO_STATUS1 WHEN 1 THEN Rtrim(PRO_CLI1)+ ' ' + Rtrim(PRO_MOD1)+' Paq. '+ PRO_PAQ1 END),'')+' '+" &
        ...
        "isnull(RTRIM(COM_MARCA1),'') + ' ' +isnull(RTRIM(COM_MARCA2),'') + ' ' + isnull(RTRIM(COM_MARCA3),'') as CMP,isnull(ACT_ESTATUSPE,'') as PE,isnull(ACT_ESTATUSPA,'') as PA,isnull(EC_ESTATUS,'') as CRE,isnull(ENG_ESTATUS,'') as ENG,isnull(ACT_ESTATUSFAC,'') as FAC,isnull(ACT_ESTATUSENT,'') as ENT" &
        " From CRM_PROSPECTOS as PROS LEFT JOIN CRM_VENDEDORES AS VEND ON PROS.PRO_IDvend = VEND.VEND_ID LEFT JOIN CRM_TAC ON PROS.PRO_ID = CRM_TAC.TAC_IDPROS left join CRM_ACTIVIDADES AS ACT ON PROS.PRO_ID = ACT.ACT_IDPROS left join CRM_COMPETIDOR AS COMP ON PROS.PRO_ID = COMP.COM_IDPROS left join CRM_ACCESORIOS AS ACC ON PROS.PRO_ID = ACC.ACC_PROSID LEFT JOIN CRM_ENGANCHE AS ENG ON PROS.PRO_ID = ENG.ENG_IDPROS LEFT JOIN CRM_EC AS EC ON PROS.PRO_ID = EC.EC_IDPROS"
...
"Select PRO_ID,isnull(RTRIM(VEND_NOMBRE),'') + ' ' + isnull(RTRIM(VEND_PATERNO),'') + ' ' + isnull(RTRIM(VEND_MATERNO),'') as VEND, Rtrim(PRO_NOMBRE)+' '+Rtrim(PRO_PATERNO)+' '+Rtrim(PRO_MATERNO) as NOMBRE ,ISNULL((CASE PRO_STATUS1 WHEN 1 THEN Rtrim(PRO_CLI1)+ ' ' + Rtrim(PRO_MOD1)+' Paq. '+ PRO_PAQ1 END),'')+' '+" &
        ...
        "isnull(ACT_ESTATUSATN,'') as ATN,isnull(ACT_ESTATUSPM,'') as PM,isnull(ACT_ESTATUSGE,'') as GE,isnull(ACc_ESTATUS,'') as ACC,isnull(ACT_ESTATUSFI,'') as FI,isnull(TAC_ESTATUS,'') as TAC," &
        "isnull(RTRIM(COM_MARCA1),'') + ' ' +isnull(RTRIM(COM_MARCA2),'') + ' ' + isnull(RTRIM(COM_MARCA3),'') as CMP,isnull(ACT_ESTATUSPE,'') as PE,isnull(ACT_ESTATUSPA,'') as PA,isnull(EC_ESTATUS,'') as CRE,isnull(ENG_ESTATUS,'') as ENG,isnull(ACT_ESTATUSFAC,'') as FAC,isnull(ACT_ESTATUSENT,'') as ENT" &
        " From CRM_PROSPECTOS as PROS LEFT JOIN CRM_VENDEDORES AS VEND ON PROS.PRO_IDvend = VEND.VEND_ID LEFT JOIN CRM_TAC ON PROS.PRO_ID = CRM_TAC.TAC_IDPROS left join CRM_ACTIVIDADES AS ACT ON PROS.PRO_ID = ACT.ACT_IDPROS left join CRM_COMPETIDOR AS COMP ON PROS.PRO_ID = COMP.COM_IDPROS left join CRM_ACCESORIOS AS ACC ON PROS.PRO_ID = ACC.ACC_PROSID LEFT JOIN CRM_ENGANCHE AS ENG ON PROS.PRO_ID = ENG.ENG_IDPROS LEFT JOIN CRM_EC AS EC ON PROS.PRO_ID = EC.EC_IDPROS WHERE PRO_NOMBRE like '" + txtnombre.Text + "' or PRO_PATERNO like '" + txtpaterno.Text + "' OR PRO_MATERNO like '" + txtmaterno.Text + "'"
        Conexion.LlenarGrilla(Sql, "CRM_PROSPECTOS")
        DvgProsperto.DataSource = Conexion.Dt
        COLOR_CELDAS()
        ColoreaAct()
    ...
    Public Sub COLOR_CELDAS()
        For A As Integer = 0 To DvgProsperto.RowCount() - 1 Step 1
            For B As Integer = 5 To 17 Step 1
                If B = 5 Then
                    'Actividad Atención
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btncanc.Click, btncanccita.Click, btncancvend.Click
        Mostrar_Datos()
    End Sub
...
"Select PRO_ID,isnull(RTRIM(VEND_NOMBRE),'') + ' ' + isnull(RTRIM(VEND_PATERNO),'') + ' ' + isnull(RTRIM(VEND_MATERNO),'') as VEND, Rtrim(PRO_NOMBRE)+' '+Rtrim(PRO_PATERNO)+' '+Rtrim(PRO_MATERNO) as NOMBRE ,ISNULL((CASE PRO_STATUS1 WHEN 1 THEN Rtrim(PRO_CLI1)+ ' ' + Rtrim(PRO_MOD1)+' Paq. '+ PRO_PAQ1 END),'')+' '+" &
        ...
        "isnull(ACT_ESTATUSATN,'') as ATN,isnull(ACT_ESTATUSPM,'') as PM,isnull(ACT_ESTATUSGE,'') as GE,isnull(ACC_ESTATUS,'') as ACC,isnull(ACT_ESTATUSFI,'') as FI,isnull(TAC_ESTATUS,'') as TAC," &
        "isnull(RTRIM(COM_MARCA1),'') + ' ' +isnull(RTRIM(COM_MARCA2),'') + ' ' + isnull(RTRIM(COM_MARCA3),'') as CMP,isnull(ACT_ESTATUSPE,'') as PE,isnull(ACT_ESTATUSPA,'') as PA,isnull(EC_ESTATUS,'') as CRE,isnull(ENG_ESTATUS,'') as ENG,isnull(ACT_ESTATUSFAC,'') as FAC,isnull(ACT_ESTATUSENT,'') as ENT" &
        " From CRM_PROSPECTOS as PROS LEFT JOIN CRM_VENDEDORES AS VEND ON PROS.PRO_IDvend = VEND.VEND_ID LEFT JOIN CRM_TAC ON PROS.PRO_ID = CRM_TAC.TAC_IDPROS left join CRM_ACTIVIDADES AS ACT ON PROS.PRO_ID = ACT.ACT_IDPROS left join CRM_COMPETIDOR AS COMP ON PROS.PRO_ID = COMP.COM_IDPROS left join CRM_ACCESORIOS AS ACC ON PROS.PRO_ID = ACC.ACC_PROSID LEFT JOIN CRM_ENGANCHE AS ENG ON PROS.PRO_ID = ENG.ENG_IDPROS LEFT JOIN CRM_EC AS EC ON PROS.PRO_ID = EC.EC_IDPROS WHERE isnull(ACT.ACT_ESTATUSFAC,'') <> 'SI' AND VEND.VEND_ID = '" & Mid(cmbvendedor.Text, (InStr(1, cmbvendedor.Text, "-") + 1)) & "'"
        Conexion.LlenarGrilla(Sql, "CRM_PROSPECTOS")
        DvgProsperto.DataSource = Conexion.Dt
        COLOR_CELDAS()
        ColoreaAct()
...
"Select PRO_ID,isnull(RTRIM(VEND_NOMBRE),'') + ' ' + isnull(RTRIM(VEND_PATERNO),'') + ' ' + isnull(RTRIM(VEND_MATERNO),'') as VEND, Rtrim(PRO_NOMBRE)+' '+Rtrim(PRO_PATERNO)+' '+Rtrim(PRO_MATERNO) as NOMBRE ,ISNULL((CASE PRO_STATUS1 WHEN 1 THEN Rtrim(PRO_CLI1)+ ' ' + Rtrim(PRO_MOD1)+' Paq. '+ PRO_PAQ1 END),'')+' '+" &
        ...
        "isnull(ACT_ESTATUSATN,'') as ATN,isnull(ACT_ESTATUSPM,'') as PM,isnull(ACT_ESTATUSGE,'') as GE,isnull(ACC_ESTATUS,'') as ACC,isnull(ACT_ESTATUSFI,'') as FI,isnull(TAC_ESTATUS,'') as TAC," &
        "isnull(RTRIM(COM_MARCA1),'') + ' ' +isnull(RTRIM(COM_MARCA2),'') + ' ' + isnull(RTRIM(COM_MARCA3),'') as CMP,isnull(ACT_ESTATUSPE,'') as PE,isnull(ACT_ESTATUSPA,'') as PA,isnull(EC_ESTATUS,'') as CRE,isnull(ENG_ESTATUS,'') as ENG,isnull(ACT_ESTATUSFAC,'') as FAC,isnull(ACT_ESTATUSENT,'') as ENT" &
        " From CRM_PROSPECTOS as PROS LEFT JOIN CRM_VENDEDORES AS VEND ON PROS.PRO_IDvend = VEND.VEND_ID LEFT JOIN CRM_TAC ON PROS.PRO_ID = CRM_TAC.TAC_IDPROS left join CRM_ACTIVIDADES AS ACT ON PROS.PRO_ID = ACT.ACT_IDPROS left join CRM_COMPETIDOR AS COMP ON PROS.PRO_ID = COMP.COM_IDPROS LEFT JOIN CRM_CITAS AS CIT ON PROS.PRO_ID = CIT.CIT_IDPROS left join CRM_ACCESORIOS AS ACC ON PROS.PRO_ID = ACC.ACC_PROSID LEFT JOIN CRM_ENGANCHE AS ENG ON PROS.PRO_ID = ENG.ENG_IDPROS LEFT JOIN CRM_EC AS EC ON PROS.PRO_ID = EC.EC_IDPROS WHERE isnull(ACT.ACT_ESTATUSFAC,'') <> 'SI' AND CIT.CIT_FECCITA = '" & dtpcita.Value & "'"
        Conexion.LlenarGrilla(Sql, "CRM_PROSPECTOS")
        DvgProsperto.DataSource = Conexion.Dt
        COLOR_CELDAS()
        ColoreaAct()
